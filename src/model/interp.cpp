...
double Interp::update_pchip(double xq)
{
    ...
    else // Evaluate on valid interval
    {
        ...
        const double s  = xq - xData_[iDim][iX];

        const double ck = (3*pchip_.del[iX] - 2*pchip_.d[iX] - pchip_.d[iX+1])/pchip_.h[iX];
        const double bk = (pchip_.d[iX] - 2*pchip_.del[iX] + pchip_.d[iX+1])/pow(pchip_.h[iX], 2);
